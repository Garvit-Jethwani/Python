...
class Test_IirFilterSetCoefficients:
    ...
    @pytest.mark.parametrize("order, a_coeffs, b_coeffs", [(2, [1.0, 0.5, 0.25], [1.0, 0.5, 0.25])])
    def test_set_coefficients_valid_order(self, order, a_coeffs, b_coeffs):
    ...
    @pytest.mark.parametrize("order, a_coeffs, b_coeffs", [(3, [0.5, 0.25], [1.0, 0.5, 0.25, 0.1])])
    def test_set_coefficients_lesser_order(self, order, a_coeffs, b_coeffs):
        filter = IIRFilter(order)
        filter.set_coefficients(a_coeffs, b_coeffs)
        assert filter.a_coeffs == [1.0] * (order + 1 - len(a_coeffs)) + a_coeffs
        assert filter.b_coeffs == b_coeffs

    @pytest.mark.parametrize("order, a_coeffs, b_coeffs", [(2, [1.0, 0.5, 0.25, 0.1], [1.0, 0.5, 0.25, 0.1])])
    def test_set_coefficients_greater_order(self, order, a_coeffs, b_coeffs):
        filter = IIRFilter(order)
        with pytest.raises(ValueError) as e:
            filter.set_coefficients(a_coeffs, b_coeffs)
        assert str(e.value) == f"Expected a_coeffs to have {order + 1} elements for {order}-order filter, got {len(a_coeffs)}"

    @pytest.mark.parametrize("order, a_coeffs, b_coeffs", [(3, [1.0, 0.5, 0.25], [1.0, 0.5, 0.25])])
    def test_set_coefficients_invalid_order(self, order, a_coeffs, b_coeffs):
        filter = IIRFilter(order)
        with pytest.raises(ValueError) as e:
            filter.set_coefficients(a_coeffs, b_coeffs)
        assert str(e.value) == f"Expected a_coeffs to have {order + 1} elements for {order}-order filter, got {len(a_coeffs)}"
